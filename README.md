...
The download software can be used independently from the photo viewer, and I run this on any old Linux box. The photo display software runs on a Raspberry Pi attached to one of the official displays and packaged into a frame enclosure (but you can run this on any Linux box connected to a display).
...
The photo viewing software is written in PyQt (v5). If you are trying to install this on a Raspberry Pi, it will fail if you try to install via ```pip```. So instead install the PyQt libraries first via the ```apt``` package manager:
...
Finally, the image cropping (to adapt to different photo frame formats) is done using the ```imagemagick``` libraries. I tried to do this using in python with ```PIL/Pillow```, but these do not have support for the .HEIC image format, so you will need to install the ```imagemagick``` tools separately. On the Raspberry Pi, this can be done via ```apt``` (other platforms can use their specific package managers e.g. ```homebrew```):
...
* frame_profile - the type of photo frame device. Currently supports ```pi``` or ```nixplay```
...
The script uses some pre-defined settings for your specific photo frame. You will likely need to edit these yourself. Open ```bin/refresh_photos``` and jump to the section marked ```### EDIT THIS SECTION TO ADD YOUR PHOTO FRAME ###```. Add a new photo frame (or edit an existing one. You must set:
...
Start the ```refresh_photos``` script. If this is the first time you are running it you will be ask you to authenticate yourself to the iCloud. This uses the familiar Apple 2-phase authentication process where you need to enter the mobile number and you will be sent a 1-time use code. Enter this here and the process should continue. The photos from the album are queried for one matching your frame (landscape/portrait). A random sample is then chosen up to the maximum number specified and the downloading begins. Once the photos are downloaded they will be cropped/rotated using the ```convert``` tool from the ```imagemagick``` suite. The cropped photos are saved in the output photo, and any temporary photos are cleaned up.
...
* ```compass```: indicates if the frame auto-detects the physical orientation of the frame. 2 compass types are supported: ```fixed``` (a hard-coded compass) or ```mpu6050``` (a popular accelerometer)
* ```rotation```: only used by the ```fixed``` compass. Defines the rotation of the frame (90, 180, 270 etc).
* ```flip_rotation```: if the angle reported by the compass should be inverted (useful for an MPU-6050 sensor that is installed back-to-front). Values: true/false.
...
The Pi is mounted on to the official Raspberry Pi display and responds to touches on the touchscreen for navigating back/forwards, bringing up popup menus or changing the slideshow. We will go into more details later on how to use the software. I did look at other options: an old monitor from eBay, or maybe some other Pi-compatible display. But really, I would with the official display - it is well-made, reasonably priced, supportes touchscreen, and...it just works.

![pi_display](pi_display.png)

Once the Pi was hooked up to the screen, I squeezed it all inside an Ikea photo frame. I actually bought a few different frame sizes at the same time. I only use the largest frame, but the other cardboard covers came in handy to place over the display and hide the edges. The Ikea frames are cheap and just about thick enough to accomodate a Pi display with a Pi stuck on the back. I had to mount it offset to the display and flipped over so the GPIO pins did not bend against the frame back. You can experiment to get it to fit.
